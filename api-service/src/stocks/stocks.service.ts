...
export class StocksService {
  ...
  public async getHistory(userId: string, page: number = 1, limit: number = 100): Promise<any> {
    const total = await this.repository.count({
      user_id: userId
    })
    const skip = (page - 1) * limit;
    const data = await this.repository.aggregate([
      { $match: { user_id: userId } },
      { $sort: { created_at: -1 } },
      { $project: { _id: 0, date: "$created_at", name: "$name", symbol: "$symbol", open: "$open", high: "$high", low: "$low", close: "$close" } },
      { $skip: skip },
      { $limit: limit }
    ]).toArray()
